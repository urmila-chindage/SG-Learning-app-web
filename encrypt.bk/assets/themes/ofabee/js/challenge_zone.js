    $(document).ready(function () {
        ...
        process_data(__challenges);
    });

    function get_categories(){
        $('#load_more a').html('Loading...');
        $.ajax({
            url: __site_url+'challenge_zone/ajax_load_challenge',
            type: "POST",
            data:{"is_ajax":true,'limit':__limit,'offset':__offset,'user_id':__user_id},
    ...
    function process_data(data){
        var rendersOutput = '';
        __challenges_recieved += Object.keys(data).length;
        $('#load_more').remove();
        $.each(data, function(ratingkey, category )
        {
            rendersOutput += render_challenge_body(category,ratingkey);
        });
        if(Object.keys(data).length == __limit){
            rendersOutput += '<div class="col-sm-12 full-width pull-left mb30" id="load_more"><a class="btn orange-flat-btn orange-course-btn more-changes-btn-padding rounded mt20 medbtn" onClick="get_categories()" href="javascript:void(0)">More Categories</a></div>';
        }
        $('#main_area').append(rendersOutput);
        if(__challenges_recieved==__total_challenges){
            $('#load_more').hide();
        }
       
    }

    function render_challenge_body(data,key){
        //console.log(data);
        var rendersHtml = '';
        if(data['challenges'].length > 0){
            rendersHtml += '<div class="row"><div class="col-xs-8 col-sm-8 col-md-8"><h3 class="all-challenge-head">Online Tests in <strong>';
            rendersHtml += data['category_name']+'</strong></h3></div><div class="col-xs-4 col-sm-4 col-md-4">';
            if(data['challenges'].length >= 4){
                rendersHtml += '<a href="'+__site_url+'challenge_zone/category/'+data['category_id']+'" class="more-challenges">View All</a>';
            }
            rendersHtml += '</div></div>';
            rendersHtml += '<div class="category-news-content"><div class="row">';
            rendersHtml += render_chellenges(data['challenges']);
            rendersHtml += '</div></div><span class="dark-line"></span>';
        }        
    ...
    function render_single_challenge(challenge){
        var rendersHtml = '';
        rendersHtml += '<div class="col-xs-4 col-sm-3 col-md-3 mb30 table-challenge mobile-challenge"><div class="challenge-inside-block shadow-box">';
