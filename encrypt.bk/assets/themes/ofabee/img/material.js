...
function renderVideoOld(video)
{
    clearInterval(__playerTimeOut);
    ...
    $('.innercontent').hide();
    $('#videocontent').show();
    __player = videojs('ofabee-video');
    __player.src({"type":"application/x-mpegURL", "src":video_path});
    if(video['ll_percentage'] < 99)
    {
        __player.currentTime (Math.round((video['ll_percentage']*video['cl_duration'])/100));
    }
    __player.play();
    if(__isPlayingDefined == false)
    {
        __isPlayingDefined = true;
        __player.on(['waiting', 'pause'], function() {
             __isPlaying = false;
            clearInterval(__playerTimeOut);
        });

        __player.on('playing', function() {
            __isPlaying = true;
            __playerTimeOut =  setInterval(function(){ 
            var percentage = ((__player.currentTime()/video['cl_duration'])*100);
            updateLecturePercentage(video['id'],  percentage);
            }, triggerTime);
        });
        
        __player.on('ended', function() {
    ...
    // because this will have no effect anyway
        if (flowplayer.support.firstframe) {
            ...
            player.bind("seek", function(e, api) {
                ...
                if(player.video.time > currentSecondLocal)
                {
...
if(lecture['descriptive']['comments'].length > 0 )
{
    for(var i =0; i < lecture['descriptive']['comments'].length; i++)
    {
        ...
        descriptiveHtml += '            <a class="ts-md-2 comment-delete-btn" onclick="deleteComment('+lecture['descriptive']['comments'][i]['comment_id']+')">X</a>';
        }
        descriptiveHtml += '        </div>';
        descriptiveHtml += '    </div>   ';        
    }
    ...
    descriptiveHtml += '        <div class="ts-md-10">';
    descriptiveHtml += '            <textarea id="descriptive_test_comment" style="resize:none;" name="message" class="small-bx" cols="" rows="" placeholder="Paste your Google drive/dropbox link and hit enter.Enter comments if any" onfocus="this.placeholder = \'\'" onblur="this.placeholder = \'Paste your Google drive/dropbox link and hit enter.Enter comments if any\'"></textarea>';
    descriptiveHtml += '        </div>';
    descriptiveHtml += '    </div> ';
    descriptiveHtml += '</div>';
    ...
    if(e.which == 13)
    {
        ...
        else
        {
            ...
            $.ajax({
                ...
                type: "POST",
                data:{"is_ajax":true, 'comment':comment, 'lecture_id':__lecture_id},
                success: function(response) {
                    var descriptiveHtml = '';
                    var data = $.parseJSON(response);
                    for(var i =0; i < data['comments'].length; i++)
                    {
                        ...
                        descriptiveHtml += '            <a class="ts-md-2 comment-delete-btn" onclick="deleteComment('+data['comments'][i]['comment_id']+')">X</a>';
                        }
                        descriptiveHtml += '        </div>';
                        descriptiveHtml += '    </div>   ';        
                    }
                    ...
                    descriptiveHtml += '        <div class="ts-md-10">';
                    descriptiveHtml += '            <textarea id="descriptive_test_comment" style="resize:none;" name="message" class="small-bx" cols="" rows="" placeholder="Enter your comment here" onfocus="this.placeholder = \'\'" onblur="this.placeholder = \'Enter your comment here\'"></textarea>';
                    descriptiveHtml += '        </div>';
                    descriptiveHtml += '    </div> ';
                    descriptiveHtml += '</div>';
	...
	function postUserComments(comment_id,value){		
	    ...
	    $.ajax({		
	            ...
	            success: function(response) {		
	                ...
	                var childstrTime     = timeSince(date);		
