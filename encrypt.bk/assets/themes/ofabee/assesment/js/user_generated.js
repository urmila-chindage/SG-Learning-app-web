...
{   
    ...
    $.ajax({
         url: __site_url+'/material/user_test_assets',
         type: "POST",
         data:{ "is_ajax":true, 'exam_id':__exam_id},
         success: function(response) {
            ...
            questionTypes += '        <h3 id="question_type_label">Question Type : MCQ</h3> ';
            questionTypes += '    </div>';
            questionTypes += '</div>';
            //end
            
            //rendering categories and question tabs
            if( __response['exam']['uga_show_categories'] == '1' && Object.keys(__response['exam']['categories']).length>0)
            {
                $.each(__response['exam']['categories'], function( index, value ) {
                ...
                $('#current_category_title').html('You are viewing <br/> '+__response['exam']['categories'][__category]['qc_category_name']);
            }
            else
            {
            ...
            $('#load_exam_button').html('Resume Exam');
...
function  loadQuestionPaper()
{
    ...
    var questionPaperIndex = Object.keys(__response['exam']['questions']);
    var exam = null;
    if(questionPaperIndex.length > 0 )
    {
        for (var key in questionPaperIndex)
        {
            ...
            {
                exam = __response['exam']['questions'][questionPaperIndex[key]];
                switch (exam['q_type'])
                {
                    case "1":
                        questionPaperHtml +='<div class="qus_sheet one">';
                        questionPaperHtml +='    <div class="question">'+exam['q_question']+'</div>';
                        ...
                        if(exam['options'].length > 0 )
                        {
                           for(var i=0;i<exam['options'].length;i++)
                           {
                                ...
                                questionPaperHtml +='            <div>'+exam['options'][i]['qo_options']+'</div></div>';
                           }
                        }
    ...
    questionPaperHtml +='<a href="javascript:void(0)" onclick="resumeChallenge()">Resume Exam</a>';
...
function renderQuestion()
{
    ...
    var exam = null;
    if(__response['exam']['uga_show_categories'] == '1' )
    {
        if(__category != null )
        {
           exam = __response['exam']['questions'][__response['exam']['category_questions'][__category][__question_index]];
            //console.log(lecture);    
        } 
    }
    else
    {
        if(Object.keys(__response['exam']['questions']).length > 0)
        {
            var currentIndex = 0;
            $.each(__response['exam']['questions'], function( index, value ) {
    ...
    if(exam != null )
    {
        ...
        {
            case "1":
                renderSingleChoice(exam);
                break;
            case "2":
                renderMultipleChoice(exam);
                break;
            case "3":
                renderSubjective(exam);
                break;
        }
        //console.log(exam);    
    } 
...
function MarkForReviewAndNext()
{
    ...
    if(temp_question_id != null)
    {
       ...
       __marked_review[temp_question_id] = temp_question_id;
        ...
        //__answered.pop(temp_question_id);
...
{
    var lecture = __response['exam']['questions'][__current_question];
...
{
    var challenge = __response['exam']['questions'][__current_question];
...
{
    var exam = __response['exam']['questions'][question_id];
    switch (exam['q_type'])
    {
        case "1":
            renderSingleChoice(exam);
...
function submitExam()
{
    ...
    $.ajax({
         url: __site_url+'/material/save_usergeneratedtest',
         type: "POST",
         data:{ "is_ajax":true, 'time_taken':__time_taken, 'answer': JSON.stringify(__answerSheet), 'exam_id':__exam_id, 'answer_time_log': JSON.stringify(__answeringTimeLog)},
         success: function(response) {
             var data = $.parseJSON(response);
             //console.log(data);
             location.href = __site_url+'/material/user_generated_test_report_item/'+data['attempt_id']
...
{		
    var totalQuestion       = Object.keys(__response['exam']['questions']).length;		
...
function renderQuestionPallette()
{
    ...
    var palletteClass = '';
        if(__response['exam']['uga_show_categories'] == '1')
        {
            if(Object.keys(__response['exam']['sl_no'][__category]).length > 0)
            {
                $.each(__response['exam']['sl_no'][__category], function( index, value ) {
                    palletteClass = '';
...
{
    __question_index++;
    if(  __response['exam']['uga_show_categories'] == '1' && typeof __response['exam']['category_questions'][__category][__question_index] == 'undefined')
    {
        var lockCategory = false;
        if(Object.keys(__response['exam']['categories']).length>0)
        {
           for (key in __response['exam']['categories'])
           {
               ...
               if (__response['exam']['categories'].hasOwnProperty(key))
               {
                   if(key==__category)
                   {
                       lockCategory = true;
...
function renderMultipleChoice(exam)
{
        __current_question = exam['id'];
        __visitedQuestions[__current_question] = __current_question;
        startAnsweringTimeLog();
    var questionHtml = '';
        questionHtml +='<div class="qus_sheet one">';
        questionHtml +='    <div class="question">'+exam['q_question']+'</div>';
        ...
        if(exam['options'].length > 0 )
        {
           for(var i=0;i<exam['options'].length;i++)
           {
                ...
                questionHtml +='            <input type="checkbox" '+(((typeof __answerSheet['answer'][exam['id']] != 'undefined') && (inArray(exam['options'][i]['id'], __answerSheet['answer'][exam['id']])))?__checked:'')+' id="option_'+exam['options'][i]['id']+'" value="'+exam['options'][i]['id']+'" class="checkBox_ question-options"> ';
                ...
                questionHtml +='            <div>'+exam['options'][i]['qo_options']+'</div></div>';
           }
        }
        questionHtml +='    </div>';
        questionHtml +='</div>';
    $('#question_content').html(questionHtml);
    $('#question_type_label').html('Question Type : '+__response['exam']['question_types'][exam['q_type']][0]);
    $('#question_number_label').html('Question No : '+$('#pallette_block_'+exam['id']).attr('data-id'));
}

function renderSingleChoice(exam)
{
        __current_question = exam['id'];
        __visitedQuestions[__current_question] = __current_question;
        startAnsweringTimeLog();
    var questionHtml = '';
        ...
        if(exam['options'].length > 0 )
        {
           ...
           {
                questionHtml +='        <div class="check">';
                questionHtml +='            <input type="radio" '+(((typeof __answerSheet['answer'][exam['id']] != 'undefined') && (exam['options'][i]['id'] == __answerSheet['answer'][exam['id']]))?__checked:'')+' name="radio_option" id="option_'+exam['options'][i]['id']+'" value="'+exam['options'][i]['id']+'" class="checkBox_ question-options"> ';
                questionHtml +='            <label for="name1" class="checkText"><span></span></label>';
                questionHtml +='            <div>'+exam['options'][i]['qo_options']+'</div></div>';
           }
        }
...
function renderSubjective(exam)
{
        __current_question = exam['id'];
        __visitedQuestions[__current_question] = __current_question;
        startAnsweringTimeLog();
    var questionHtml = '';
        ...
        questionHtml +='    <div class="question">'+exam['q_question']+'</div>';
        questionHtml +='    <div class="options_main">';
        questionHtml +='        <div class="check"><textarea id="subjective_answer" style="width: 100%;height: 200px">'+((typeof __answerSheet['answer'][exam['id']] != 'undefined' && __answerSheet['answer'][exam['id']] !== null)?__answerSheet['answer'][exam['id']]:'')+'</textarea></div>';
        questionHtml +='    </div>';
        questionHtml +='</div>';
    $('#question_content').html(questionHtml);
    $('#question_type_label').html('Question Type : '+__response['exam']['question_types'][exam['q_type']][0]);
...
function initDomActions()
{
    ...
    var duration = __response['exam']['uga_duration']*60;
    startTimer(duration);
    //End
