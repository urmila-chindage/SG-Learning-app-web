...
function renderEmptyObject(){
    ...
    renderHtml += '<p>No topics to show</p></div>';
...
function render_forum_object(topic_obj){
    var renderHtml = '';
    __current_page_items = Object.keys(topic_obj).length;
    $.each(topic_obj, function(topic_key, topic )
    {
        ...
        renderHtml += '</span></a>'+renderOptions(topic['id'])+'</li>';
...
function renderOptions(topic_id){
    ...
    renderHtml += '<li><a href="javascript:void(0);" onclick="deleteTopic('+topic_id+')">Delete</a></li></ul></div>';
...
function pagination(links){
    ...
    if(links>1){
        ...
        renderHtml += '<span class="forum-pagination-page" id="footer_page_details">'+__total_forums+' topics | page <strong>'+1+'</strong> of <strong>'+links+'</strong></span>';

        $('.links-pagination').html(renderHtml);
        $('.first-link').hide();
        $('.previous-link').hide();
        $('.forum-and-page').html(__total_forums+' '+topic_text+' | page <strong>1</strong> of <strong>'+links+'</strong>');
    }else{
        renderHtml += '<span class="forum-pagination-page" id="footer_page_details">'+__total_forums+' '+topic_text+' | page <strong>'+1+'</strong> of <strong>'+links+'</strong></span>';
        $('.links-pagination').html(renderHtml);
        $('.forum-and-page').html(__total_forums+' '+topic_text+' | page <strong>1</strong> of <strong>'+links+'</strong>');
    }
}
...
function deleteTopic(topic_id){
    ...
    $('#delete_main').attr('onclick','deleteTopicConfirmed('+topic_id+')');
}

function deleteTopicConfirmed(topic_id){
    $('#DeleteModal').modal('hide');
    $.ajax({
        url: __site_url+'forum/delete_ajax_topic',
        type: "POST",
        data:{"is_ajax":true,'topic_id':topic_id,'admin_id':0,'per_page':__limit,'forum_id':__forum_id},
        success: function(response) {
            var data = $.parseJSON(response);
            if(data['success'] == true){
                __num_pages = data['pages'];
                __num_pages_search = __num_pages;
                __total_forums = data['topic_count'];
                if(__current_page == 1){
                    paginate(1);
                }else{
                    if(__current_page_items>1){
