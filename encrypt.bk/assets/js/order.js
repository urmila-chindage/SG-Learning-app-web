...
function renderOrderRow(data) {
    ...
    if(data.ph_payment_mode == 1) {
        var paymentMode = 'Online';
    } else if(data.ph_payment_mode == 3) {
        var paymentMode = 'Offline';
    } else {
        var paymentMode = 'Free Course';
    }
        
    if (data) {
        var invoiceLink = 'javascript:void(0)';
        var target      = '';
        ...
        orderHtml += '  <div class="rTableCell rowdivision"><div class="ellipsis-style user-type">'+paymentMode+'</div></div>';
        ...
        if(data.ph_item_amount_received > 0){
            invoiceLink = adminUrl+'orders/pdf/'+data.id;
            target      = 'target="_blank"';
        }
        orderHtml += '  <div class="rTableCell rowdivision"><div class="ellipsis-style user-type"><label class="pull-right label label-info" style="margin: 1px 2px 0px 0px;"><a class="" style="color: #fff;" href="'+invoiceLink+'" '+target+'>Order details </a></label></div></div>';
