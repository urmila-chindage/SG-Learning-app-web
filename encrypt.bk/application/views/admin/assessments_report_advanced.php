    ...
    <div class="course-report" id="course_report_container_wrapper" <?php echo ((!empty($lectures)) && (!empty($subscribers)))?'':'style="visibility: hidden;"'  ?> >
    	   ...
    	   <!-- Generate student grade report -->
            <div class="stud_grade_report" style="visibility:visible;">
                <div class="report_title text-center"><h4>Generate Student Quiz Report</h4></div>
                ...
                <div class="half-width">
                    <div class="course-filter">
                        <select class="course-report-filter" id="assignment_filter" onchange="updateUrlState();" >
                            ...
                            <?php foreach($quizes as $quiz): ?>
                                <option <?php echo ($quiz['id']==$quiz_id)?$selected:'' ?> value="<?php echo $quiz['id'] ?>" ><?php echo $quiz['cl_lecture_name'] ?></option>
                            <?php endforeach; ?>
                            <?php endif; ?>
                        </select>
                    </div>
                    ...
                    <div class="pull-right filter-btn">
                        <button class="btn btn-green selected" onclick="exportAssessmentReport()">EXPORT REPORT</button>
...
function courseFilter(){
    ...
    $.ajax({
            url: webConfigs('admin_url')+'report/get_assessments',
            type: "POST",
            ...
            success: function(response) {
                ...
                {
                    $('#assignment_filter').html('');
                    var assignmentHtml      = '<option value="all" >Select Quiz</option>';
...
function getReport(){
    var pathname                = '/<?php echo $this->config->item('admin_folder').'/report/assessments_report'; ?>';
    ...
    var assessment_id           = $("#assignment_filter").val();
    ...
    if((assessment_id=='0') || (assessment_id=='all')){
        var messageObject = {
        'body':'Please choose quiz',
        'button_yes':'OK', 
        'button_no':'CANCEL'
        };
        callback_warning_modal(messageObject);
    ...
    if(course_id != '' || assessment_id != '' || institute_id != '' || batch_id != '' || filter_by != '') {
        link += '?';
    }
    if(course_id != '') {
