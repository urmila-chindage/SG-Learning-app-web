...
li.ui-timepicker-am {text-transform: uppercase;}
li.ui-timepicker-pm {text-transform: uppercase;}
...
<form id="access_restriction_form" onsubmit="return validateForm()" method="POST" action="<?php echo admin_url('coursebuilder/save_access_restriction') ?>">
        ...
        <div class="form-group clearfix">
            ...
            <div class="col-sm-3 no-padding">
                <input class="form-control text-center" <?php echo $disabled ?> value="<?php echo isset($restriction_course_percentage['percentage']) ? $restriction_course_percentage['percentage'] : '' ?>" type="text" maxlength="3" onkeypress="return isNumber(event)" name="restriction[course_percentage][percentage]" placeholder="%">
            </div>
        </div>
        <?php
        $checked        = '';
        ...
        <div class="form-group clearfix">
            ...
            <div class="col-sm-5">
                <input id="date_avail_from" class="form-control text-center" <?php echo $disabled ?> value="<?php echo (isset($restriction_available_from['date']) ? $restriction_available_from['date'] : '') ?>" type="text" autocomplete="off" name="restriction[available_from][date]" placeholder="dd-mm-yyyy" readonly="readonly">
            </div>
        ...
        <div class="form-group clearfix">
            ...
            <div class="col-sm-5">
                <input id="date_avail_to" class="form-control text-center" <?php echo $disabled ?> value="<?php echo (isset($restriction_available_till['date']) ? $restriction_available_till['date'] : '') ?>" type="text" autocomplete="off" name="restriction[available_till][date]" placeholder="dd-mm-yyyy" readonly="readonly">
            </div>
                ...
                <?php foreach ($restriction_activities as $key => $activity):  ?>
                    ...
                    <div class="row activities-row" data-id="<?php echo $key ?>" id="activity_row_<?php echo $key ?>">
                        ...
                        <div class="col-sm-5 lecture-dropdown-wrapper">
                            <select class="form-control lecture-dropdown customeRules" data-lecture-id="<?php echo $activity['lecture_id'] ?>" name="restriction[activities][<?php echo $key ?>][lecture_id]"></select>
                        </div>
                        <div class="col-sm-5">
                        ...
                        <div class="col-sm-1 no-padding">
                            <?php $disabled = ($activity['action'] != 'complete_with_percenatge') ? 'disabled="disabled"' : '';?>
        ...
        <div class="text-right">
            ...
            <input type="hidden" name="assessment_id" value="<?php echo $test['assessment_id'] ?>">
            <input type="hidden" name="redirect_to" id="redirect_to" value="<?php echo current_url() ?>">
        </div>
</form>
...
<script>
   /* var __activityCount         = Number('<?php //echo count($restriction_activities) ?>')+1;
    ...
    $(document).ready(function(){
        ...
        $("input[name='restriction[available_from][active]']").change(function(){
            $("input[name='restriction[available_from][date]'], input[name='restriction[available_from][time]']").val('');
            if($(this).prop('checked') == true) {
                $("input[name='restriction[available_from][date]'], input[name='restriction[available_from][time]']").removeAttr('disabled');
            } else {
                $("input[name='restriction[available_from][date]'], input[name='restriction[available_from][time]']").attr('disabled', 'disabled');
            }
        });

        $("input[name='restriction[available_till][active]']").change(function(){
            $("input[name='restriction[available_till][date]'], input[name='restriction[available_till][time]']").val('');
            if($(this).prop('checked') == true) {
                $("input[name='restriction[available_till][date]'], input[name='restriction[available_till][time]']").removeAttr('disabled');
            } else {
                $("input[name='restriction[available_till][date]'], input[name='restriction[available_till][time]']").attr('disabled', 'disabled');
            }
        });
        ...
        $("input[name='restriction[available_from][time]'], input[name='restriction[available_till][time]']").timepicker({});
    ...
    function saveAccessRestriction() {
        ...
        $('.customeRules').each(function(indexes, items) {
            rulesArray.push($(items).val());
        });
            
        for(var i = 0; i < rulesArray.length; ++i) {
            if(!rulesUsing[rulesArray[i]])
            rulesUsing[rulesArray[i]] = 0;
            ++rulesUsing[rulesArray[i]];
        }
        ...
        if($(__actionListItems).length > 0 ) {
            $(__actionListItems).each(function(index, item) {
                ...
                if($(item).val() == 'complete_with_percenatge') {
                    var completionPercentage = $.trim($("input[name='restriction[activities]["+rowId+"][percentage]']").val());
                    if(isNaN(completionPercentage) == false ) {
                        if( completionPercentage <= 0 || completionPercentage > 100 ) {
                            errorCount++;
        ...
        if(rulesUsing.length > 0){
            $(rulesUsing).each(function(index, value) {
                if(value > 1){
                    errorCount++;
                    errorMessage.push(`You are selected a lecture more than one time for <b>Rule ${index}</b>`);
                }
            });
        }
    ...
    function generateActivity() {
        ...
        //console.log(__activityCount, __lectureOptionHtml);
        renderingActivity = true;
        var activityHtml  = '';
            activityHtml += '<div class="row activities-row" data-id="'+__activityCount+'" id="activity_row_'+__activityCount+'">';
            activityHtml += ' <label class="activity-count-label">Rule '+__activityCount+'</label>';
            activityHtml += ' <div class="clearfix"></div>';
            activityHtml += ' <div class="col-sm-5 lecture-dropdown-wrapper">';
            activityHtml += ' <select class="form-control lecture-dropdown customeRules" name="restriction[activities]['+__activityCount+'][lecture_id]">'+__lectureOptionHtml+'</select>';
            activityHtml += ' </div>';
            activityHtml += ' <div class="col-sm-5">';
            activityHtml += ' <select class="form-control lecture-action" name="restriction[activities]['+__activityCount+'][action]">'+getLectureActionHtml(0)+'</select>';
            activityHtml += ' </div>';
    ...
    $(document).on( 'change', '.lecture-action', function(){
        var activityId = $(this).parents('.activities-row').attr('data-id');
        if($(this).val() == 'complete_with_percenatge') {
             $("input[name='restriction[activities]["+activityId+"][percentage]']").removeAttr('disabled');
        } else {
            $("input[name='restriction[activities]["+activityId+"][percentage]']").attr('disabled', 'disabled');
        }
    ...
    function getLectureActionHtml(type) {
        ...
        switch(type) {
            case 1:
                actionHtml = '<option value="complete">Must Complete</option><option value="complete_with_percenatge">Must Complete with %</option>';
                break;
            default:
                actionHtml = '<option value="complete">Must Complete</option>';
                break;
    ...
    function validateForm(){
        var GivenDate       = $('#date_avail_from').val();
        var GivenDateTo     = $('#date_avail_to').val();
        if ($('#avail-from').is(':checked')) {
            if(GivenDate!=null||GivenDate==''||GivenDate=='undefined')
            {
                myDate              = GivenDate.split("-");
        ...
        if((GivenDate!=null||GivenDate!=''||GivenDate!='undefined') && (GivenDateTo!=null||GivenDateTo!=''||GivenDateTo!='undefined'))
        {
           // alert('hj');
            var start_date                  = toDate($('#date_avail_from').val());
            var end_date                    = toDate($('#date_avail_to').val());
            start_date.setHours(0,0,0,0);
            end_date.setHours(0,0,0,0);
            if(start_date > end_date)
            {
                errorMessage      ='Available till date should be greater than from date.';
                var messageObject = {
                'body':errorMessage,
                'button_yes':'OK',
                };
        ...
        $("input[name='restriction[available_till][date]").datepicker({
            language: 'en',
            minDate: new Date(),
            // dateFormat: 'dd-mm-yy', //date_avail_from//from_time
            onSelect: function(dateText, inst) {
                var sel_date            = new Date(dateText);
                var today_date          = new Date();
                var today_date_second   = new Date(today_date.getFullYear(),today_date.getMonth(),today_date.getDate(),0,0,0,0);
                ...
                if(sel_date.getDate() == today_date_second.getDate()){
                    ...
                    var current_time = today_date.getHours();
                    var timedate1 = $('#date_avail_from').val();
                    var timedate2 = $('#from_time').val();
                    console.log(timedate1,'---',timedate2);
