                ...
                <div class="rTableRow d-flex align-center justify-between ">
                    ...
                    <div class="rTableCell dropdown">
                        ...
                        <ul class="dropdown-menu white">
                            <li><a href="javascript:void(0)" onclick="filter_user_by('all')" id="filer_dropdown_list_all"><?php echo lang('all_students') ?></a></li>
                            ...
                            <!-- <li><a href="javascript:void(0)" onclick="filter_user_by('suspended')" id="filer_dropdown_list_suspended">Pending Approval</a></li> -->
                        </ul>
                    </div>
                    ...
                    <div class="rTableCell" style="border:0px;">
                        ...
                        <div id="course_bulk" class="btn-group lecture-control btn-right-align" style="margin-top: 0px; display: none;">
                            ...
                            <ul class="dropdown-menu pull-right" role="menu">
                                ...
                                <!-- <li>
                                    <a onclick="removeUserFromBundle('0')" href="javascript:void(0)"><?php echo lang('remove_from_bundle') ?></a>
                                </li> -->
                    ...
                    <div class="rTableCell dropdown">
                        <div class="bundle-bulk-action">
                            <span class="dropdown-tigger" data-toggle="dropdown">
                                <span class="label-text"><i class="icon icon-down-arrow"></i></span>
                            </span>
                            <ul class="dropdown-menu pull-right" role="menu">
                                <li><a href="<?php echo admin_url('bundle/enroll_users/' . $course['id']) ?>" data-toggle="modal">Enroll</a></li>
                                <?php if(in_array($this->__access['view'], $this->__bundle_student_enrollment_privilege)): ?>
                                    <li id="bundleExport" <?php echo count($enrolled_users) > 0 ? '' : 'style="display:none"'?>><a href="javascript:void(0)" onclick="exportSubscribersReport()" data-toggle="modal">Export</a></li>
                                <?php endif;?>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div class="rTableCell">
                        <?php if(in_array($this->__access['add'], $this->__bundle_student_enrollment_privilege)): ?>
                        <div class="col-sm-12">
                            <a href="<?php echo admin_url('bundle/enroll_users/' . $course['id']) ?>" class="btn btn-violet" >ENROLL<ripples></ripples></a>
                        </div>
                        <?php endif; ?>
                    </div> */?>
                    <!-- enroll button ends -->
            ...
            <?php
                ...
                <div >
                    <div class="student-export-info">
                        <h4 class="right-top-header user-count text-right">
                            <?php
                            $user_html = '';
                ...
                <div class="table course-cont only-course rTable"  id="user_row_wrapper" >
                    <?php if (!empty($enrolled_users) && $filter == '') {?>
                        <?php foreach ($enrolled_users as $user): ?>
                            <div class="rTableRow user-listing-row" id="user_row_<?php echo $user['bs_user_id'] ?>" data-name="<?php echo $user['us_name'] ?>" data-email="<?php echo $user['us_email'] ?>">
                                <div class="rTableCell">
                                    <input type="checkbox" class="user-checkbox" value="<?php echo $user['bs_user_id'] ?>" id="user_details_<?php echo $user['bs_user_id'] ?>">
                                    ...
                                    <div class="manage-stud-list" style="display:inline !important;line-height: 11px;position:relative;top: 5px;">
                                        <a href="<?php echo admin_url('user/profile/' . $user['bs_user_id']) ?>" >
                                ...
                                <!-- <div class="rTableCell pad0" style="min-width:100px;" id="progress_bar_user_<?php echo $user['bs_user_id'] ?>">
                                    <div class="cent-algn-txt float-r">
                                        <a href="javascrip:void(0)" class="link-style"><?php echo $user['bs_percentage'] ?>%</a>
                                    </div>
                                    <div class="progress sml-progress stud-course-progress">
                                        <div class="progress-bar" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $user['bs_percentage'] ?>%;">
                                            <span class="sr-only"><?php echo $user['bs_percentage'] ?>% <?php echo lang('complete') ?></span>
                                        </div>
                                    </div>
                                </div> -->
                                <div class="td-dropdown rTableCell">
                                    <div class="btn-group lecture-control">
                                        ...
                                        <ul class="dropdown-menu pull-right" role="menu">
                                            ...
                                            <li>
                                                <a href="javascript:void(0)" data-toggle="modal" onclick="sendMessageToUser('<?php echo $user['bs_user_id'] ?>')" id="invite_course_user_individual"><?php echo lang('send_message') ?></a>
                                            </li>
                                            <!-- <li>
                                                <a onclick="removeUserFromBundle('<?php echo $bundle_id?>','<?php echo $user['bs_user_id'] ?>','<?php echo $user['us_name'] ?>')" href="javascript:void(0)"><?php echo lang('remove_from_bundle') ?></a>
                                            </li> -->
                                            <?php //if(in_array($this->__access['edit'], $this->__bundle_student_enrollment_privilege)): ?>
                                            <!-- <li>
                                                <a href="javascript:void(0);" id="extend-time_<?php echo $user['bs_user_id'] ?>" data-toggle="modal" data-target="#extend-validity" onclick="extendValidityForCourse('<?php echo $user['bs_bundle_id'] ?>','<?php echo $user['bs_user_id'] ?>','<?php echo $user['bs_end_date'] ?>','<?php echo $user['us_name'] ?>')">Change Validity Period</a>
                                            </li>
    
                                            <li id="complete_btn_<?php echo $user['bs_user_id'] ?>">
                                                <?php
                                                //if($user['bs_percentage']>=0&&$user['bs_percentage']!=100){
                                                ?>
                                                    <a href="javascript:void(0)" onclick="setAsComplete('<?php echo $user['bs_bundle_id'] ?>','<?php echo $user['bs_user_id'] ?>','<?php echo $user['us_name'] ?>')"><?php echo lang('set_as_complete') ?></a>
                                                <?php
                                                //}
                                                ?>
                                            </li> -->

                                            <!-- <li id="reset_btn_<?php echo $user['bs_user_id'] ?>">
                                                <?php
                                                //if($user['bs_percentage']>=0&&$user['bs_percentage']!=0){
                                                ?>
                                                    <a href="javascript:void(0)" onclick="resetResult('<?php echo $user['bs_bundle_id'] ?>','<?php echo $user['bs_user_id'] ?>','<?php echo $user['us_name'] ?>')"><?php echo lang('reset_result') ?></a>
                                                <?php
                                                //}
                                                ?>
                                            </li> -->
                                            <!-- <li id="status_btn_<?php echo $user['bs_user_id'] ?>">
                                                <?php $cb_status = ($user['bs_approved'] == 1) ? 'suspend' : 'approve';?>
                                                <?php $cb_action = $cb_status?>
                                                <a href="javascript:void(0);"  onclick="changeSubscriptionStatus('<?php echo $user['bs_user_id'] ?>', '<?php echo $user['bs_bundle_id'] ?>', '<?php echo ($user['bs_approved'] == '1') ? '0' : '1'; ?>')" ><?php echo lang($cb_status) ?></a>
                                            </li> -->
                                            
                                            <?php //if(in_array($this->__access['delete'], $this->__bundle_student_enrollment_privilege)): ?>
                                            <!-- <li>
                                                <a href="javascript:void(0)"  onclick="removeUserFromCourse('<?php echo $user['bs_bundle_id'] ?>','<?php echo $user['bs_user_id'] ?>','<?php echo addslashes($user['us_name']) ?>')"><?php echo lang('remove_from_bundle') ?></a>
                                            </li> -->
                                            <?php //endif; ?>
                                        </ul>
                                    </div>
...
?>
    ...
    <!-- ############################# --> <!-- START -->
        ...
        const __previlages__        = atob('<?php echo base64_encode(json_encode($this->__bundle_student_enrollment_privilege)) ?>');
        </script>

    <script type="text/javascript" src="<?php echo assets_url() ?>js/bundle_user.js"></script>
        ...
        <script>
            ...
            var __course_title = '<?php echo $title; ?>';
                ...
                if(url=='success'){
                    var messageObject = {
                        'body': 'Students enrolled to bundle',
                        'button_yes': 'OK',
                    };
