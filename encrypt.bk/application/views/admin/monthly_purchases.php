...
<section class="courses-tab base-cont-top">
    <ol class="nav nav-tabs offa-tab">
        ...
        <!-- active tab end -->
        <li>
            <a href="<?php echo admin_url(); ?>finance/monthly_teachers_shares">Teacher Percentage</a>
            <span class="active-arrow" style="background: rgb(255, 255, 255);"></span>
        </li>
    </ol>
</section>
                ...
                <div class="rTable content-nav-tbl" style="">
                    <div class="rTableRow">
                        ...
                        <div class="rTableCell sales-search">
                            <div class="input-group">
                                <input type="text" id="course_keyword" class="form-control srch_txt" placeholder="Search by Name">
                        ...
                        <div class="rTableCell sales-export">
                            <a class="btn btn-green selected mt8" onclick="exportMonthlySales()"><i class="icon icon-upload"></i> EXPORT <ripples></ripples></a>
                        </div>
...
<script>
    ...
    var __purchasesObject = atob('<?php echo base64_encode(json_encode($monthly_purchases)) ?>');    
    ...
    function getPurchases()
    {
        ...
        $.ajax({
            url: __adminUrl+'finance/monthly_purchases_json',
            type: "POST",
            ...
            success: function(response) {
                ...
                if(data['error']==false)
                {
                    ...
                    {
                        $('#monthly_sales_wrapper').html('');
                        $('#monthly_sales_wrapper').html(renderMonthlyPurchasesHtml(data['monthly_purchases']));
                    }
                    else
                    {
                        $('#monthly_sales_wrapper').append(renderMonthlyPurchasesHtml(data['monthly_purchases']));
                    }
                    __start = false;
                    __offset++;
                }
        ...
        if(Object.keys(purchases).length > 0 )
        {
            $.each(purchases, function(key, purchase )
            {
                if(initialPurchaseDetail == true && __start == true)
                {
                    ...
                    getPurchasesDetails(purchase['ph_item_id'], __activeMonth);
                }
                purchasesHtml += '<div onclick="getPurchasesDetails('+purchase['ph_item_id']+', \''+ __activeMonth+'\')" class="rTableRow" data-item-id="'+purchase['ph_item_id']+'" id="purchase_row_'+purchase['ph_item_id']+'">';
                purchasesHtml += '    <div class="rTableCell">';
                purchasesHtml += '        <span class="icon-wrap-round green sales-icons"><i class="icon icon-graduation-cap"></i></span>';
                purchasesHtml += '        <span class="earning-course-content">';
                purchasesHtml += '            <a href="javascript:void(0)" class="normal-base-color grp-click-fn"><span class="earning-course-name">'+purchase['cb_title']+'</span></a>';
                purchasesHtml += '        </span>';
                purchasesHtml += '        <span class="label-active group-total sales-total rel3">'+purchase['total_sales']+' Sales</span>';
                purchasesHtml += '        <span class="group-total sales-total pull-right">'+purchase['total_amount']+'  &#8377;</span>';
                purchasesHtml += '    </div>';
            ...
            purchasesHtml = renderPopUpMessage('error','No records found.');
        }
        __start = false;
    ...
    function getPurchasesDetails(course_id, period)
    {
        $('#monthly_purchase_details_wrapper').html('<div class="rTableRow "><h3>Loading.....</h3></div>');
        $('.active-user-custom').removeClass('active-table');
        $('#purchase_row_'+course_id+' .active-user-custom').addClass('active-table');
        $.ajax({
            url: __adminUrl+'finance/monthly_purchases_details_json',
            type: "POST",
            data:{"is_ajax":true, "course_id":course_id, 'period':period},
            success: function(response) {
                var data = $.parseJSON(response);
                $('#monthly_purchase_details_wrapper').html(renderMonthlyPurchasesDetailsHtml(data['monthly_purchases_details']));
            }
        });
    }
    function renderMonthlyPurchasesDetailsHtml(purchasesDetails)
    {
        ...
        if(Object.keys(purchasesDetails).length > 0 )
        {
            ...
            {
                purchasesDetailsHtml += '<div onclick="generateInvoice('+purchaseDetail['id']+')" class="rTableRow sales-invoice-modal"> ';
                ...
                purchasesDetailsHtml += '    <div class="rTableCell sales-candidate">'+purchaseDetail['us_name']+'</div>';
                purchasesDetailsHtml += '    <div class="rTableCell sales-amt">'+purchaseDetail['total_amount']+'  &#8377;</div>';
    ...
    {
        var keyword  = $('#course_keyword').val();
       location.href = __adminUrl+'finance/export_monthly_purchases/'+btoa(__activeMonth+'#'+keyword);
    }
    ...
    function generateInvoice(paymentId)
    {
        ...
        $.ajax({
            url: __adminUrl+'finance/payment_details',
            type: "POST",
            data:{"is_ajax":true, "payment_id":paymentId},
            success: function(response) {
                var data    = $.parseJSON(response);
                ...
                var invoiceHtml = '';
                    ...
                    invoiceHtml += '            <div class="col-sm-8">'+payment['us_name']+'</div>';
                    invoiceHtml += '        </div>  ';                              
                    invoiceHtml += '        <div class="row pb10">';
                    ...
                    invoiceHtml += '            <div class="col-sm-8">'+payment['total_amount']+'</div>';
                    invoiceHtml += '        </div>  ';                              
                    invoiceHtml += '        <div class="row pb10">';
                    ...
                    invoiceHtml += '            <div class="col-sm-8">'+payment['ph_teacher_share']+'</div>';
                    invoiceHtml += '        </div>  ';       
                    if(typeof payment['shares']!= 'undefined' )
                    {
                        invoiceHtml += '<div class="row pb10">      ';
                        invoiceHtml += '    <table class="table" >';
                        invoiceHtml += '        <thead><tr><th colspan="2" align="center">Teacher Share Details</th></tr><thead>';
                        invoiceHtml += '        <tbody>';
                        if(Object.keys(payment['shares']).length > 0 )
                        {
                            invoiceHtml += '            <tr><td>Teacher Name</td><td>Share</td></tr>';
                            $.each(payment['shares'], function(key, share )
                            {
                                invoiceHtml += '            <tr><td>'+share['us_name']+'</td><td>'+share['ps_amount']+'</td></tr>';                        
                            });
                        }
                        invoiceHtml += '        </tbody>';
                        invoiceHtml += '    </table>';
                ...
                $('#export_invoice').click({"payment_id": paymentId}, exportInvoice);  
            }
        });
    }
    ...
    function exportInvoice(param)
    {
        ...
        a.href=__adminUrl+'finance/pdf/'+param.data.payment_id;
        a.target = '_blank';
