        ...
        <div class="container">
            <div class="container-reduce-width">
                ...
                <ul class="funda-rank clearfix">
                    ...
                    <li class="funda-rank-list"><span class="funda-rank-dark span-mam">Mark scored <span class="mark-funda-number" id="attempt_mark"><?php echo (($my_mark)?round($my_mark, 2):'0') ?></span></span></li>
                    ...
                    <li class="view-rank-list-wraper-alterd">
                        ...
                        <ul class="view-mark-list-ul">
                            <li><a href="<?php echo site_url('material/assesment_report_item/'.$my_attempt_id) ?>" class="orange-flat-btn orange-btn-alterd-for-next-page">View test report</a></li>
                        </ul>
                        <?php endif; ?>
                    </li>
                </ul>          
...
<script>
    ...
    var __assessmentObject      = $.parseJSON(atob('<?php echo base64_encode(json_encode($assessment)) ?>'));
    ...
    { 
        ...
        if(Object.keys(attempts).length > 0 )
        {
            $.each(attempts, function(attemptKey, attempt )
            {
                ...
                ranksHtml += '<tr class="tabel-white-strip tabel-white-strip-left-text '+((__userId == attempt['aa_user_id'])?'focused-row':'')+'" id="rank_wrapper_'+attempt['aa_user_id']+'">';
                ...
                ranksHtml += '    <td class="table-sub-rows table-sub-rows-altr text-center"><span class="text-middle">'+((attempt['total_mark']!=null)?Number(attempt['total_mark']).toFixed(2):'')+'</span></td>';
                ranksHtml += '    <td class="table-sub-rows table-sub-rows-altr text-center"><span class="text-middle">'+secondsToHms(attempt['aa_duration'])+'</span></td>';
                ranksHtml += '    <td class="table-sub-rows table-sub-rows-altr text-center"><span class="text-middle"><input '+((__userId == attempt['aa_user_id'])?__checked+' '+__disabled:'')+' id="user_'+attempt['id']+'" value="'+attempt['id']+'" class="user-rank-selected" name="user_'+attempt['id']+'" type="checkbox"><label class="label-narrow label-narrow-altr" for="user_'+attempt['id']+'"><span></span></label></span></td>';
                ranksHtml += '</tr>';
                if(__userId == attempt['aa_user_id'])
                {
    ...
    {
        $.ajax({
            url: __site_url+'report/assessment_json',
            type: "POST",
    ...
    function compareResult()
    {
        if(Object.keys(__compareIds).length > 1 && Object.keys(__compareIds).length < 5)
        {
           $('body').append('<a id="assessment_compare_btn_link" href="'+__site_url+'report/assessment_compare/'+btoa(JSON.stringify(__compareIds))+'"></a>');
