...
class Pending_order extends CI_Controller
{
    ...
    { //print_r($_GET); die;
        ...
        $breadcrumb[]               = array( 'label' => lang('pending_orders'), 'link' => admin_url('pending_order'), 'active' => 'active', 'icon' => '' );
        $data['breadcrumb']         = $breadcrumb;
        $data['title']              = lang('pending_orders');
        ...
        $order_param['filter']      = ($filter != null)? $filter : 'pending';
        $order_param['ph_status']   = array('2');
        if($keyword != null)
        {
        ...
        $data['controller']        = 'pending_order';
        $data['admin']             = $this->__loggedInUser;
        $this->load->view($this->config->item('admin_folder').'/pending_orders', $data);
    }
    ...
    public function export_sales_report($param = false)
    {
        ...
        $filter_param['filter']                       = $param['filter'] ? $param['filter'] : 'pending';
        ...
        $filter_param['ph_status']                    = array('2');
        ...
        $data['status']                               = $filter_param['filter'] == 'processing' ? 'Pending' :  $filter_param['filter'];
